...
<form onSubmit={formik.handleSubmit} className="max-w-md mt-8 mx-auto">
  ...
  <div className="relative z-0 w-full  mb-5 group">
    ...
    <label htmlFor="floating_email" className="peer-focus:font-medium  absolute left-0 text-md text-gray-500  duration-300 transform -translate-y-6 scale-75 top-1 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-emerald-600 peer-focus:dark:text-emerald-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 ">Email Address
     <span className='ms-64'>    <i className="fa-solid text-xl  ms-10 text-slate-400 fa-at"></i>   </span>     </label>
...
<button type="submit" className="text-white bg-emerald-700 px-11 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-emborder-emerald-300 font-bold rounded-lg text-sm w-full sm:w-auto  py-2.5 text-center ">{isLoding ? <i className='fas fa-spinner fa-spin'></i> : "Login" }</button>
</div>


<div className='mt-8 text-center'><Link to="/ForgetPassword"><span className='text-emerald-600 hover:text-white underline '>Forget Password?</span></Link> </div>
<div className='mt-3 text-center'><Link to="/Register"><span className='text-emerald-600 hover:text-white underline '>Don't Have Account? Register Now</span></Link> </div>
